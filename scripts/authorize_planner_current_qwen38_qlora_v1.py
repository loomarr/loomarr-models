...
def main() -> None:
    parser = argparse.ArgumentParser(description="Authorize the exact current QLoRA plan")
...
def authorize(plan_commit: str, reference: str, authorized_at: str) -> None:
    ...
    if plan.sourceCommit != plan_commit or plan.trainingAuthorized:
        raise ValueError("current QLoRA plan is not the exact clean unauthorized commit")
    authorization = {
        "schemaVersion": 1,
