...
from loomarr_models.current_baseline import load_config, preflight
from loomarr_models.experiment import PreflightError


def main() -> None:
    parser = argparse.ArgumentParser(description="Fail-closed current-contract Qwen stock baseline")
    parser.add_argument(
        "--config",
        type=Path,
    ...
    try:
        ...
        from loomarr_models.current_stock_runtime import run_baseline
...
def _arm_timeout(seconds: int) -> None:
    if not hasattr(signal, "SIGALRM"):
        raise PreflightError("live current stock baseline requires SIGALRM wall-clock enforcement")

    def expired(_signum: int, _frame: object) -> None:
        raise TimeoutError(f"current stock baseline exceeded {seconds} seconds")
