...
from loomarr_models.v4_review import derive_review, empty_decision, load_review_decisions
from run_planner_v4_review import CONFIG_PATH, build_plan


PUBLIC_ROOT = ROOT / "reviews/planner-v4-delta/publications"
TRACE_COUNT = 60
OBSERVATION_COUNT = TRACE_COUNT * 2


def main() -> None:
    parser = argparse.ArgumentParser(description="Publish planner v4 delta review evidence")
...
def publish(plan: Any) -> dict[str, Any]:
    ...
    if public.exists():
        raise ModelReviewError("published v4 review directory already exists")
    ...
    if set(manifest) != expected_manifest_fields or manifest.get("status") != "complete":
        raise ModelReviewError("run manifest is not an exact complete v4 review manifest")
    expected_identity = {
    ...
    if any(manifest.get(key) != value for key, value in expected_identity.items()):
        raise ModelReviewError("run manifest differs from the committed v4 review plan")
    ...
    if hashlib.sha256(attestation_bytes).hexdigest() != manifest["attestationsSha256"]:
        raise ModelReviewError("v4 review attestation digest mismatch")
    if hashlib.sha256(invalid_bytes).hexdigest() != manifest["invalidReviewsSha256"]:
        raise ModelReviewError("v4 review invalid-observation digest mismatch")
    ...
    if (
        ...
        or len(attestations) + len(invalid_reviews) != OBSERVATION_COUNT
    ):
        raise ModelReviewError("published run must contain exactly 120 review observations")

    entries = manifest["callArtifacts"]
    if not isinstance(entries, list) or len(entries) != len(plan.requests):
        raise ModelReviewError("v4 review call artifact manifest is incomplete")
    ...
    for request, entry in zip(plan.requests, entries, strict=True):
        ...
        if not isinstance(entry, dict) or entry.get("stem") != stem:
            raise ModelReviewError("v4 review call artifact order differs from the plan")
    ...
    if ordered_attestations != attestations or ordered_invalid != invalid_reviews:
        raise ModelReviewError("v4 review observations differ from exact call order")
    if str(actual_cost) != manifest["actualCostUsd"] or actual_cost > Decimal(plan.reservationUsd):
        raise ModelReviewError("v4 review cost differs from settlements or exceeds reservation")

    decisions, escalations = derive_decisions(attestations, invalid_reviews, plan)
    decision_bytes = b"".join(canonical(decision) + b"\n" for decision in decisions)
    trace_ids = corpus.trace_ids()
    current = load_review_decisions(corpus.REVIEW_PATH, trace_ids)
    if list(current.values()) != [empty_decision(trace_id) for trace_id in trace_ids]:
        raise ModelReviewError("canonical v4 decisions are not pristine")
    budget = _object(corpus.ROOT / "budgets/external-spend-v1.json")
    ...
    result = {
        ...
        "approved": TRACE_COUNT - len(escalations),
    ...
    (corpus.ROOT / "budgets/external-spend-v1.json").write_bytes(_pretty(settled_budget))
    return result
...
def promote_approved(config_path: Path) -> dict[str, Any]:
    ...
    if review_id != "planner-v4-delta-review-v1":
        raise ModelReviewError("unexpected v4 review publication identity")
    public = PUBLIC_ROOT / review_id
    publication = _object(public / "publication.json")
    if publication.get("approved") != TRACE_COUNT or publication.get("escalations") != 0:
        raise ModelReviewError("only a unanimous 60-trace publication may be promoted")
    decision_path = public / "decisions.jsonl"
    decision_bytes = decision_path.read_bytes()
    if hashlib.sha256(decision_bytes).hexdigest() != publication.get("decisionsSha256"):
        raise ModelReviewError("published v4 decisions digest mismatch")
    trace_ids = corpus.trace_ids()
    decisions = load_review_decisions(decision_path, trace_ids)
    if any(
        derive_review(decision, require_complete=True).status != "approved"
        for decision in decisions.values()
    ):
        raise ModelReviewError("published v4 decisions are not unanimously approved")
    current = load_review_decisions(corpus.REVIEW_PATH, trace_ids)
    if list(current.values()) != [empty_decision(trace_id) for trace_id in trace_ids]:
        raise ModelReviewError("canonical v4 decisions are not pristine")
    corpus.REVIEW_PATH.write_bytes(decision_bytes)
    subprocess.run([sys.executable, "scripts/build_planner_v4_delta.py"], cwd=ROOT, check=True)
    return {
        "reviewId": review_id,
...
def derive_decisions(
    ...
    if len(trace_ids) != TRACE_COUNT or len(set(trace_ids)) != TRACE_COUNT:
        raise ModelReviewError("primary plan does not cover exactly 60 v4 traces")
    ...
    for trace_id in trace_ids:
        ...
        for role in ("primary", "secondary"):
            ...
            if attestation is None:
                raise ModelReviewError(f"{trace_id}: missing {role} v4 review")
...
def settle_budget(budget: dict[str, Any], cost: Decimal, plan: Any) -> dict[str, Any]:
    ...
    if posted + outstanding != committed or str(committed) != plan.committedSpendUsd:
        raise ModelReviewError("budget changed after the committed v4 preflight")
    if cost > Decimal(plan.reservationUsd) or committed + cost > authorization:
        raise ModelReviewError("settled v4 review cost exceeds authorization")
    result = dict(budget)
...
def _validate_invalid_observation(
    ...
    if any(actual.get(key) != value for key, value in expected.items()):
        raise ModelReviewError("invalid v4 observation differs from raw evidence")
    if set(actual) != {*expected, "reviewedAt"} or not isinstance(actual.get("reviewedAt"), str):
        raise ModelReviewError("invalid v4 observation fields are not exact")
...
def _validate_attestation(
    ...
    if any(actual.get(key) != value for key, value in parsed.items()):
        raise ModelReviewError(f"{stem}: v4 attestation differs from raw response")
    if (
        ...
        raise ModelReviewError(f"{stem}: v4 attestation settlement evidence is invalid")
