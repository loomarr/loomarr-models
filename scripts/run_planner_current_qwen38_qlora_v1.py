...
from loomarr_models.current_training import load_config, preflight
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run one fail-closed current-contract QLoRA training")
...
def _arm_timeout(seconds: int) -> None:
    if not hasattr(signal, "SIGALRM"):
        raise PreflightError("live current QLoRA training requires SIGALRM wall-clock enforcement")
