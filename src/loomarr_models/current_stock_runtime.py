...
def run_baseline(
    ...
    if platform.system() != "Linux" or platform.machine() != "x86_64":
        raise PreflightError("live current stock baseline requires Linux amd64")
    ...
    for index, case in enumerate(cases, start=1):
        ...
        print(f"current stock baseline progress: case={index}/{len(cases)} caseId={case['caseId']}", flush=True)
    ...
    summary = summarize_current_candidate(CANDIDATE_ID, results, peak_vram_bytes=peak)
    ...
    manifest = {
        ...
        "artifacts": {
            ...
            "generations": {"path": generations_path.name, "sha256": hashlib.sha256(generations_bytes).hexdigest()},
        },
        "summary": summary,
        "decision": decision,
